...
# Formato: {StopPointNo: {"action": "enable", "stop_time": secondi}}
OPERATIONS = {
    370: {
        "action": "enable",
    ...
    if target_lr is None:
        ...
        if not tp:
            ...
            for stop_no, config in OPERATIONS.items():
                ...
                print("\nOperazione: %s fermata %d (StopTime=%d sec)" % (operation.upper(), stop_no, stop_time))
                print("-" * 80)
                ...
                if operation == "enable":
                    ...
                    # RICALCOLA Arr e Dep con nuovo PreRunTime
                    arr_370 = prev_dep + pre_run_time
                    dep_370 = arr_370 + stop_time
